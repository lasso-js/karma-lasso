...
var framework = function(emitter, config, logger) {
    ...
    config.lasso.plugins.forEach(function(plugin, index) {
        if (plugin === 'lasso-require') {
            requirePlugin = {
                plugin: 'lasso-require'
            };
            pluginIndex = index;
            spliceCount = 1;
        } else if (plugin.plugin === 'lasso-require') {
            requirePlugin = plugin;
            pluginIndex = index;
            spliceCount = 1;
        }
    });
    // normalize the require plugin
    requirePlugin.config = requirePlugin.config || {};
    requirePlugin.config.transforms = requirePlugin.config.transforms || [];
    // include the autowatch transform if watch is true
    if (config.autoWatch !== false) {
        requirePlugin.config.transforms.unshift('karma-lasso/lib/transforms/watcher');
    }
    // include the instrumentation transform if the coverage is to be done
    if (config.lasso.coverage) {
        requirePlugin.config.transforms.unshift('karma-lasso/lib/transforms/coverage');
    }
